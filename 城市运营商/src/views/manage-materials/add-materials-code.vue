...
export default {
  ...
  methods: {
    ...
    getCurrentSkuItem (item) {
      ...
      // if (parentIndex === 0 || parentIndex === 1 || parentIndex === 2 || parentIndex === 3 || parentIndex === 4) {
    ...
    removeCustomSkuItem (payload) {
      console.log(payload)
      let { parentIndex } = payload
      // if (parentIndex === 0 || parentIndex === 1) {
      this.goodsSkuList[parentIndex].sku.forEach(skuItem => {
        if (payload.id === skuItem.id) {
          skuItem.checked = false
          this.goodsSkuCombineData[parentIndex].list = []
        }
      })
    ...
    /* eslint-enable */
    commfirCode () {
      console.log(this.goodsSkuCombineData)
      let catId = this.idList.split(',')
      ...
      if (!this.goodsSkuCombineData.length) {
        ...
        let arr = []
        this.goodsSkuCombineData.forEach(item => {
          item.list.forEach(listItem => {
            // let listName = listItem.require
            if (listItem.require === 'noRequire' || listItem.require === 'isRequire') {
              listItem.type = listItem.require
              arr.push(listItem)
            } else {
              obj[listItem.require] = JSON.stringify({ 'attr_id': listItem.attr_id, 'id': listItem.id, 'is_custom': listItem.is_custom, 'val': listItem.val })
            }
          })
        })
        obj.require = ''
        arr.forEach(arrItem => {
          obj.require += ',' + JSON.stringify({ 'attr_id': arrItem.attr_id, 'id': arrItem.id, 'type': arrItem.type, 'val': arrItem.val })
        })
