...
export default {
  ...
  methods: {
    ...
    // 获取ios数据
    getIosData () {
      let url = 'analytics/channelDetailApp'
      let obj = {
        page: this.page,
      ...
      this.$http(url, obj).then(res => {
        if (res.code === this.$ok) {
          ...
          this.singularNumber = res.data.panel.open
    ...
    handlerData (data) {
      data.forEach((item, index) => {
        ...
        item.fourTextT = item.count4
        item.fiveTextT = item.count5
        item.sixTextT = item.count6
      })
    },
    // 搜所
    getSearch () {
    ...
    handleDownload () {
      ...
      require.ensure([], () => {
        ...
        const tHeader = ['日期', '下载用户数', '首次访问用户数', '注册用户数', '审核开通用户数', '付费开通用户数', '发布商品用户数']
        const filterVal = ['date_time', 'count1', 'count2', 'count3', 'count4', 'count5', 'count6']
    ...
    // 发布商品用户数
    sixTable (row) {
      if (row.count6 === 0) return
      this.infoName = '发布商品用户数'
      this.type = 5
      this.infoTime = row.date_time
      this.tableColumns = [
        {
    ...
    // 请求接口
    getDate () {
      let url = 'analytics/channelDetailListApp'
      let obj = {
        channel_id: this.queryId,
        type: this.type,
