...
export default {
  ...
  methods: {
    ...
    getTableData () {
      ...
      let url = 'analytics/channelListApp'
      let obj = {
        page,
        pageSize,
        op_city: this.cityId
    ...
    typeSubmit (row, type) {
      ...
      if (type === 3) {
        let url = 'http://test-cityapi.cgs18.com/v1/analytics/downloadChannelQrcode?channel_id=' + row.id
      ...
      if (type === 2) {
        this.$router.push({ path: '/manage-statistics/merchant-statistics-detail', query: { id: row.id, dataTime: row.created_detail } })
      }
    },
    changePage (page) {
      this.page = page
    ...
    giveUp (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http('analytics/addChannelApp', obj).then(res => {
            if (res.code === this.$ok) {
              this.$message({
                message: '提交成功',
                type: 'success'
