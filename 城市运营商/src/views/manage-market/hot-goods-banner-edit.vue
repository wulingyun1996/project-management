<template>
  <div class="index-banner-edit-container" v-if="flag">
    ...
    <div class="index-banner-img-title">
      ...
      <span>图片尺寸750*280，建议格式：'.png', '.jpeg', '.jpg'，不超过400KB</span>
    </div>
...
export default {
  ...
  methods: {
    ...
    getEditDetail () {
      ...
      this.$http(url, obj).then(res => {
        if (res.code === this.$ok) {
          ...
          // ------------------- 添加商品 -----------------------
          if (type_style === 1) {
            let { id, image_path, product_code, title } = res.data.goods_info
            let obj = {}
            obj.id = id
            obj.src = image_path
          ...
          // ------------------- 添加联盟活动 -----------------------
          if (type_style === 4) {
            let { start_at, end_at, union_name, union_title } = res.data.act_info
            let obj = {}
            obj.id = act_url
    ...
    validateCommonFileds () {
      ...
      if (this.endTime <= initTime(this.startTime)) {
