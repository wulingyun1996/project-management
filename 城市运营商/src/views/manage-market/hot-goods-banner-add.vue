<template>
  <div class="index-banner-add-container">
    ...
    <div class="index-banner-img-title">
      ...
      <span>图片建议：尺寸750*280，不大于400KB</span>
    </div>
      ...
      <div class="choose-goods-wrapper">
        ...
        <div class="choose-goods-body">
          <base-table
            ...
            :tableHeight="300"
            :componentList="allianceComponentList"
...
export default {
  ...
  methods: {
    ...
    comfirmSubmitContent () {
      ...
      let url = 'adv/bannerAdd'
      /* eslint-disable */
      let obj = {
        title: this.adName,
        start_time: this.startTime.substring(0, this.startTime.length - 3),
        end_time: this.endTime.substring(0, this.endTime.length - 3),
        image: this.imgUrl,
        type_style: ''
      }
      ...
      if (contentType === '1') {
        ...
        // obj.pic_and_text = JSON.stringify([this.editContent])
      ...
      if (contentType === '4') {
        obj.type_style = 4
        obj.act_url = this.allianceChoosedTableData[0].id
      }
      /* eslint-enable */
      this.$http(url, obj).then(res => {
        if (res.code === this.$ok) {
          this.$router.push({ name: 'HotGoods', query: { index: '3' } })
    ...
    getAllianceList () {
      ...
      this.$http(url, obj).then(res => {
        if (res.code === this.$ok) {
          ...
          this.allianceTotal = this.allianceTableData.length
        }
        // this.$mes(res.message, 4)
      })
    },
