<template>
  <div class="add-hot-goods-banner" v-if="flag">
    ...
    <!-- <div class="activity-banner">
      ...
      <div class="upload-banner">
        <base-upload-img @uploadFile="uploadBanner" @beforeUpload="beforeUpload" :imgUrl="imgUrl"></base-upload-img>
    ...
    <div class="activity-banner">
      ...
      <div class="upload-banner">
        <base-upload-img @uploadFile="uploadHome" :limitedSize='2048' :imgUrl="imgHome"></base-upload-img>
        <p>【尺寸大小 750*280，建议格式：'.png', '.jpeg', '.jpg'，不超过2M 】</p>
      </div>
    </div>
    ...
    <!-- <div class="label-description">
      ...
      <base-upload-img @uploadFile="uploadLabel" @beforeUpload="beforeUploadLabel" :imgUrl="customLabel"></base-upload-img>
    </div> -->
    <!-- 提交新增与编辑 -->
  ...
  methods: {
    getBannerDetail () {
      ...
      this.$http(url).then(res => {
        if (res.code === this.$ok) {
          this.bannerName = res.data.hotadv.title
          this.imgUrl = res.data.hotadv.image
          // 首页广告
          this.imgHome = res.data.hotindex.image
          // 爆款标签
          let hotLabel = res.data.hotlabel.image
          let flag = false
          this.labelList.some((item, index) => {
            if (hotLabel === item) {
              flag = true
              this.imgLabel = index
              return true
    ...
    comfirmSubmitContent () {
      ...
      this.$http(url, obj).then(res => {
        if (res.code === this.$ok) {
          this.$mes('添加成功')
          // this.$router.back()
          this.$router.push({ name: 'HotGoods', query: { index: '2' } })
          // return
        }
        // this.$mes(res.message, 4)
      })
