      ...
      <div class="statistics-wrapper">
        ...
        <base-scroll :has-more="true">
          <ul>
            <!-- <div class="itemInfo">
              <ul>
                <li v-for="itemInfo in statisticsInfoData" :key="itemInfo.id">
                  <p>{{ itemInfo.count }}</p>
                  <p>{{ itemInfo.title }}</p>
                </li>
              </ul>
            ...
            <li
              ...
              <div class="sale-explain-wrapper" v-show="saleGuidePopup" v-if="item.id === 7">
                ...
                <div class="explain-body">此活动商家内所有推广员的销售业绩总金额</div>
              </div>
              <!-- 带单金额弹窗 -->
              ...
              <div class="money-explain-wrapper" v-show="moneyGuidePopup" v-if="item.id === 9">
                ...
                <div class="explain-body">此活动商家内所有推广员为其他商家带单的金额</div>
              </div>
            </li>
          </ul>
        </base-scroll>
...
export default {
  ...
  methods: {
    ...
    getStatisticsData (item) {
      ...
      this.$http(url, obj).then(res => {
        if (res.code === this.$ok) {
          ...
          // this.statisticsInfoData = res.data.splice(0, 2)
    ...
    getStatisticsDataList () {
      let url = 'shopAnalytics/getCountList'
    ...
    viewAllianceStatisticsDetail (item) {
      ...
      if (id === 1 || id === 2) return
      this.$router.push({ name: 'ShopStatisticsDetail', query: { id, actId: this.actId } })
    },
    // 弹窗事件
