<template>
  <div class="cardtwo-condition">
    ...
    <div class="equity">
      <i class="shape"></i>
      <span>特约权益</span> 
      <span class="rights">二级特约采购师可享受的店铺折扣权益</span>
    </div>
    <div class="discount">
      <span>折扣</span>
      <group>
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        let obj = {
          two_amount: this.input_amount,
          two_count: this.input_count,
          two_discount: this.input_discount
        }
      ...
      setNum() {
        ...
        if (arrZero !== null) {
          if (+this.$route.query.twoMoney !== 0) {
              ...
              if (arr) {
                if (+arr[0] < this.$route.query.twoCount) {
                  if (+this.input_discount >= 7.0) {
                    if (+this.input_discount < +this.$route.query.num) {
                      if (+this.input_discount < +this.$route.query.twoNum) {
                        this.showPop('二级折扣数值不能小于一级折扣数值')
                      } else {
                        this.loadData()
                      }
                    } else {
                      this.showPop('二级折扣数值不能大于三级折扣数值')
                    }
                  } else {
                    this.showPop('最低折扣不能低于7.0')
                  }
                } else {
                  this.showPop('二级交易次数不能大于一级交易次数')
                }
              } else {
                this.showPop('交易次数必须为大于0的整数，最多7位')
              }
            } else {
              this.showPop('二级交易额不能大于一级交易额')
            }
          } else {
            console.log('没设置一级特约卡')
            let reg = /^[1-9]\d{0,6}$/
            let arr = String(this.input_count).match(reg)
            if (arr) {
              if (+this.input_discount >= 7.0) {
