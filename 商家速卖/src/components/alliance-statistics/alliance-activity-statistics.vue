  ...
  <div class="alliance-activity-statistics-container">
    ...
    <div class="alliance-activity-statistics-wrapper">
      <div class="statistics-wrapper">
        <div class="establish-activity-guide" v-if="isInitialStatus" @click="goEstablishActivity">
        ...
        <base-scroll :has-more="true">
          <ul>
            <li
              ...
              <p>{{ item.count }}</p>
              <!-- 销售金额弹窗 -->
              <i class="icon-sale-guide" @click.stop="showSaleGuidePopup(false)" v-if="item.id === 7"></i>
...
export default {
  ...
  created () {
    this.getSelectStatisticsData().then(() => {
  ...
  methods: {
    ...
    getStatisticsData (item) {
      this.activityId = item ? item.id : this.activityId
      let url = 'analytics/getHome'
      let obj = {
        type: 2,
    ...
    getStatisticsDataList () {
      let url = 'analytics/getListCount'
    ...
    viewAllianceStatisticsDetail (item) {
      let { id, click } = item
      let { activityId, isInitialStatus } = this
      if (!click || isInitialStatus) return
      this.$router.push({ name: 'AllianceStatisticsDetail', query: { id, activityId } })
    },
    // 弹窗事件
