<template>
  <div class="apply-container">
        ...
        <div class="form">
          ...
          <div class="form-group-addr">
            ...
            <div class="address" @click="showPopup">
              <input id="address" type="text" placeholder="选择后圈子/商家/活动仅在该城市展示" v-model="serviceCitys" readonly/>
          ...
          <div class="form-group txt">
            ...
            <div class="description">
              <textarea
                ...
                @blur="restoreBodyHeight"></textarea>
              <div class="txt-num">
                <span><b>{{ intructionLength }}</b>/300</span>
              </div>
            </div>
          </div>
          <div class="form-group-pic">
          ...
          <div class="apply-pic">
            <div class="photo" :class="{no: company_img}" @click="upLoad(3)">
    ...
    <div ref="button" class="button" :class="{ active: isActive }" @click="submit">
      <span>提交</span>
    </div>
...
export default {
  ...
  computed: {
    isActive () {
      return this.name && this.tel && this.brand && this.introduction && this.city.length && this.id_front && this.id_back && this.company_img
    },
    brandLenth () {
  ...
  methods: {
    ...
    submit () {
      ...
      if (!regBrandRes) {
        this.$vux.toast.text('品牌与品牌之间请用“、”分开', 'middle')
        return false
      }
    ...
    applyAlliance () {
      ...
      loadData(url, obj).then((res) => {
        if (res.code === STATUS_OK) {
          ...
          // this.$root.allianceName = this.name
          return
        }
