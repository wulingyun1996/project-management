<template>
  <div class="quote-goods">
    <div v-if="isShow" class="showData">
      <div class="quote-top"><i></i>产品引用选择多个则确认引用后，直接成功引用为商品。选择单个产品引用时，确认引用可编辑该商品信息后再发布为商品。</div>
      <div class="quote-content">
        <div class="quote-title">
          <div class="left">选择需引用的厂家产品</div>
          <div class="right" @click="isFullCheck(fullCheck)"><span>全选</span><check-icon :value.sync="fullCheck"></check-icon></div>
        </div>
        <div class="quote-list">
          <scroll ref="scroll" class="main-content" :data="quoteList" 
          :pulldown="pulldown" :pullup="pullup" @pulldown="pulldownRefresh" :beforeScroll="beforeScroll" @scrollToEnd="searchMore" @beforeScroll="listScroll">
            <div>
              <Loading :class="refresh ? 'show' : 'hide'"></Loading>
              <div v-show="!noData && loading">
                <div class="quote-all">
                  <div class="quote-div" v-for="item in quoteList">
                    <div class="pic-box">
                    ...
                    <div class="text-box">
                      ...
                      <div class="choose-icon" @click="changeCheck(item.id, item.is_check)"><check-icon :value.sync="item.is_check"></check-icon></div>
                    </div>
                  </div>
                </div>
  ...
  export default {
    ...
    methods: {
      ...
      _checkMore(data) {
        ...
        this.isFullCheck(this.fullCheck)
      },
      listScroll() {
      ...
      isFullCheck (curVal) {
        if (curVal === true) {
          this.chooseArr = []
          this.quoteList.map((val, index) => {
            if (val.is_check === false) {
              this.$set(val, 'is_check', true)
            }
            this.chooseArr.push(val.id)
          })
        } else {
          this.quoteList.map((val, index) => {
            if (val.is_check === true) {
              this.$set(val, 'is_check', false)
            }
          })
          this.chooseArr = []
        }
      ...
      changeCheck (id, value) {
        if (value === true) {
          ...
          this.chooseArr.splice(this.chooseArr.indexOf(id), 1)
          this.fullCheck = false
        }
        if (this.chooseArr.length === this.quoteList.length) {
          this.fullCheck = true
        } else {
          this.fullCheck = false
        }
      ...
      showConfirm() {
        if (this.chooseArr.length > 1) {
          let URL = 'goods/batchQuoteProduct'
          let obj = {
            ids: this.chooseArr
          }
          loadDataPost(URL, obj).then((res) => {
        ...
        } else if (this.chooseArr.length === 0) {
          this.showPop('请选择需引用的产品')
          return false
        } else {
          // 跳转至“发布新建商品”页编辑该产品
          this.$router.push({
            path: `/edit-goods/${this.chooseArr[0]}`,
            query: {
              is: 0
            }
          })
