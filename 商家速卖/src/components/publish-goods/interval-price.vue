<template>
  <base-senior-popup width="100%" height="100%" ref="interval" class="interval-price-container">
    ...
    <base-footer text="确定" @footBtnClick="setSave"></base-footer>
  </base-senior-popup>
</template>

<script>
import { Group, XInput } from 'vux'
import baseFooter from 'base/foot-btn/foot-btn-two'
import baseIntervalPrice from './section-price'
export default {
  props: {
    postInter: {
      type: Array,
  ...
  data() {
    return {
      ...
      sectionPrice: [],
      unit: '',
      title: this.isEditNormalGoods ? '编辑区间' : '区间报价'
    }
  },
  created() {
  ...
  methods: {
    ...
    setSave() {
      ...
      this.$emit('getGoodsIntervalInfo', this.postData)
      this.$router.back()
    },
    addInput() {
      this.itemObj.push({
