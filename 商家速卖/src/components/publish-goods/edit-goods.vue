<template>
  <transition name="has-data">
    <div class="max-height" v-if="showEditGoods">
      ...
      <base-scroll
        ...
        <!-- <div class="banner">
          <Slider :goodImgs="imaList" v-if="imaList && imaList.length > 0" @goCheckActive="goCheckActive" @goAddActive="goAddActive"></Slider>
          <div class="add-banner" v-else @click="goAddActive"></div>
        </div> -->
        <div class="publish-container">
          <div class="publish-wrapper">
            ...
            <router-link :to="`/edit-goods/${goodsId}/select-category`" tag="div" class="publish-item publish-arrow">
              <label>
                <span class="item-star">*</span>
            ...
            <router-link :to="`/edit-goods/${goodsId}/select-brand`" tag="div" class="publish-item publish-arrow" v-show="brandList.length > 0">
              <label :class="{active: isChangedBrand}">
              ...
              <input :class="{active: isChangedBrand}" type="text" name="" v-model="brandObj.val" value="" readonly="readonly" />
            </router-link>
            ...
            <router-link :to="`/edit-goods/${goodsId}/select-specification-price`" tag="div" class="publish-item publish-arrow" v-show="isShowCombSpec">
              <label>
                <span class="item-star no-necessary"></span>
            ...
            <!-- <router-link
              :to="{path: `/edit-goods/${goodsId}/select-interval-price`, query: {unit: this.unitChild}}"
              tag="div"
              v-if="sectionPriceArr.length > 0 || intervalQuo"
              class="publish-item publish-arrow">
              <label>
                <span class="item-star">*</span>
      ...
      <div class="foot-div" ref="button">
        <div class="push-btn" @click="dataSubmit(0)">{{ btnText }}</div>
      </div>
...
import { splitMoney, judgeObjectLen } from 'commons'
let tempBrandList = [
  {
    categoryLevel: '店铺品牌',
    categorySymbol: '店铺品牌',
...
export default {
  ...
  computed: {
    // 是否从引用页面跳转
    isQuotationGoods () {
      return Number(this.$route.query.is) === 0
    },
    specIsShow() {
    ...
    // 佣金
    commission () {
      // if (this.partInChoose[0] === '参与') {
      //   if (this.specialCard && this.specialCard.one_discount) {
      //     let {init, zero} = splitMoney((this.recommonPrice - this.buyPrice) * (1 + this.specialCard.one_discount / 100))
      //     let res = `${init}.${zero}`
      //     if (parseFloat(res) > 0) {
      //       return res
      //     }
  ...
  created () {
    this.btnText = this.isQuotationGoods ? '立即引用' : '立即发布'
    try {
      this.initGoodsAttr().then(value => {
        this.getEditGoodsDetail(value)
      })
    } catch (e) {
  ...
  methods: {
    ...
    async initGoodsAttr() {
      // 初始化获取编辑数据 toto
      const editUrl = this.isQuotationGoods ? 'goods/quoteProduct' : 'goods/editGoods'
      let editObj = {
    ...
    getEditGoodsDetail (value) {
      // 折扣
      this.specialCard = value.data.specialCard
      // 是否在pc编辑过
      this.isPc = value.data.productObject.is_pc
      // 保存父组件数据
      let cateStr = ''
      let ids = ''
      if (value.data.cateArr.length > 0) {
      ...
      // 区间报价数据
      let secArr = []
      if (value.data.priceInterval.length > 0) {
        this.intervalQuo = true
        ...
        this.intervalQuo = false
      }
      this.sectionPriceArr = secArr
      this.unitChild = this.unitChoose[0]
      this.is_draft = value.data.productObject.is_draft
      // 特约卡是否显示
      this.isCan = value.data.has_cart
      // 是否参加特约卡
      this.partInChoose = this.isQuotationGoods && this.isCan ? ['参与'] : (value.data.productObject.is_special === 1 ? ['参与'] : ['不参与'])
      this.showEditGoods = true
    },
    // 原生 -- start
    goAddActive() {
    ...
    dataSubmit(num) {
      ...
      if (ischeck) {
        ...
        let URL = 'goods/save'
        let specRemoveArr = ['interval', 'price', 'purchasePrice', 'promotionPrice'] // 规格报价去除属性
    ...
    checkGoodsInfo() {
      ...
      if (this.brandList.length > 0) {
        if (this.brandList[0].require === 'isRequire') {
          if (!this.brandObj.val || this.isChangedBrand) {
            this.showPop('请选择品牌')
            return false
          }
        }
...
.publice-goods
  ...
  .banner
    ...
    .add-banner
      height: 280px;/*px*/
      background: $color-f4 url('./add-banner.png') no-repeat center center
      background-size: 100px 100px;/*px*/
  .publish-container
    width: 100%
    .publish-wrapper
