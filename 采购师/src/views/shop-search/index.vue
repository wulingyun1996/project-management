  ...
  <div class="shop-search-container">
    ...
    <base-header :is-show-title="false">
      <div class="header-center" slot="center">
        ...
        <!-- <input type="text" v-model="shopName"  v-focus placeholder="搜索店铺名称"> -->
        <input
        id="phone"
        type="text"
        placeholder="搜索店铺名称 "
    ...
    <div class="search-content">
      ...
      <div class="search-list">
        <ul>
          <li v-for="(item, index) in HistoryList" :key="index" @click="goSearList(item)">{{ item }}</li>
        </ul>
      </div>
    </div>
...
export default {
  ...
  created () {
    ...
    if (currentDev === 'Android') {
      ...
      this.HistoryList = hintInfo.split(',')
    }
    if (currentDev === 'iOS') {
  ...
  methods: {
    getSearch () {
      ...
      this.$http('adv/shopSearch', { shop_name: this.shopName, page: 1, op_city: getItem('opCity') }).then(res => {
        ...
        if (!this.noData.length) {
          ...
          if (this.HistoryList.length > 0) {
            ...
            if (this.HistoryList.indexOf(this.shopName) !== -1) {
              this.HistoryList.splice(this.HistoryList.indexOf(this.shopName), 1)
              this.HistoryList.unshift(this.shopName)
            } else {
              this.HistoryList.unshift(this.shopName)
            }
          } else {
          ...
          let currentDev = dev()
          if (currentDev === 'Android') {
            let list = this.HistoryList.join()
          ...
          this.$router.push({ path: '/search-shop-list', query: { shopName: this.shopName } })
        }
      })
    },
    getClose () {
    ...
    goSearList (item) {
      this.$router.push({ path: '/search-shop-list', query: { shopName: item } })
    },
    // 原生回调
    callbackNativeLocalCache (data) {
      if (data === '' || !data) return
      this.HistoryList = data.split(',')
    }
  }
}
</script>
