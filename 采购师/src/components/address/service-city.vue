      ...
      <div class="service-city-body">
        <div class="province-body" v-show="showProvinceList">
          <base-senior-scroll :hasMore="true" :data="provinceList" :beforeScroll="true" @beforeScroll="beforeScroll">
            <div class="province-item" :id="provinceitem.name" v-for="(provinceitem, index) in provinceList" @click="getCityChildren(provinceitem.value)" :key="index">{{ provinceitem.name }}</div>
          </base-senior-scroll>
        </div>
        <div class="city-body" v-show="isChoosedProvince">
          <base-senior-scroll :hasMore="true" :data="cityList">
            <div class="city-item" v-for="cityItem in cityList" @click="getAreaChildren(cityItem.value)" :key="cityItem">{{ cityItem.name }}</div>
          </base-senior-scroll>
        </div>
        <div class="area-body" v-show="isChoosedCity">
          <base-senior-scroll :hasMore="true" :data="areaList">
            <div class="area-item" v-for="areaItem in areaList" @click="getAreaItem(areaItem.value)" :key="areaItem">{{ areaItem.name }}</div>
          </base-senior-scroll>
        </div>
      </div>
    </div>
...
<script>
import BasePopupTransition from 'components/pop/popup-transition'
import BaseSeniorScroll from 'components/scroll/senior-scroll'
